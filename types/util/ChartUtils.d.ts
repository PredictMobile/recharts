...
import { AxisType, BaseAxisProps, DataKey, LayoutType, PolarLayoutType, NumberDomain, TickItem, CategoricalDomain, StackOffsetType, Margin, ChartOffset, XAxisMap } from './types';
...
export declare const getMainColorOfGraphicItem: (item: {
    type: {
        displayName: string;
    };
    props: {
...
export declare const appendOffsetOfLegend: (offset: ChartOffset, props: {
    width?: number;
    margin: Margin;
    children?: ReactNode[];
}, legendBox: BoundingBox | null) => ChartOffset;
...
export declare const getStackedDataOfItem: <StackedData>(item: ReactElement, stackGroups: Record<StackId, GenericChildStackGroup<StackedData>>) => StackedData | null;
...
export declare const parseDomainOfCategoryAxis: <T>(specifiedDomain: ReadonlyArray<T> | undefined, calculatedDomain: ReadonlyArray<T>, axisChild: ReactElement) => ReadonlyArray<T>;
export declare const getTooltipItem: (graphicalItem: {
    type: {
        displayName: string;
    };
    props: {
...
export declare const isAxisLTR: (axisMap: XAxisMap) => boolean;
